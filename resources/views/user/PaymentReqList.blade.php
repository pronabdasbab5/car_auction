...
<section id='detailed'>
		<div class="container">
			<div class='row '><div class='col-sm-12 col-md-7 col-lg-7 wow fadeInLeft'>
					<h2>Payment Request List</h2>
				</div>
				</div>@foreach($data as $key => $cat)
				<div class=' col-lg-6 wow live-auction fadeInLeft'>
					...
					<div class='row'>
						...
						<!-- {{Session::get('user_id')}} -->
						@if($cat['msg'])
							<h4><span  class="icon fa fa-bell"  style="font-size: 20px!important;"></span> {{$cat['msg']}}</h4>
						@endif
						
						@if($cat['amount'])
							<h4><span  class="icon fa fa-money"  style="font-size: 20px!important;"></span> {{$cat['amount']}}</h4>
						@endif
